...
- SpacetimeDB core concepts skill (`spacetimedb-concepts`) — architecture, tables, reducers, subscriptions, event tables, identity, and the Five Zen Principles
- SpacetimeDB Rust module skill (`spacetimedb-rust`) — server module development with tables, reducers, scheduled ticks, identity auth, error handling, and procedures (from official ClockworkLabs repo)
- SpacetimeDB TypeScript SDK skill (`spacetimedb-typescript`) — client connection, subscriptions, React integration, server-side module dev, event tables, and scheduled tables (from official ClockworkLabs repo)
- Ralph Loop game dev orchestrator skill (`ralph-loop-gamedev`) — multi-agent loop with 3 specialized agents (Implementation, Code Review + Tests, Playwright Evaluation), self-healing bug fix cycle, file-based state machine (ROADMAP.md + PHASE_TRACKER.md), and SpacetimeDB build/publish/generate integration
- PROMPT.md and PHASE_TRACKER.md templates for setting up Ralph Loop in new projects

### Changed

- Skill count updated from 8 to 12
...
[1.4.0]: https://github.com/chongdashu/vibejam-starter-pack/compare/v1.3.0...v1.4.0
